...
from bulbea._util.const import (
    ...
    SHARE_ACCEPTED_FILE_FORMATS
)
from bulbea._util.color import Color
from bulbea._util.file  import File
from bulbea.error       import VALUE_ERROR_SHARE_FILE_FORMAT
import bulbea as bb
...
def _validate_share_file_format(format_, raise_err = False):
    _check_str(format_, raise_err = True)

    if format_ not in SHARE_ACCEPTED_FILE_FORMATS:
        if raise_err:
            raise ValueError(VALUE_ERROR_SHARE_FILE_FORMAT.format(format_ = format_))
        else:
            return False
    else:
        return True
...
class Share(Entity):
    ...
    def save(self, format_ = 'csv', filename = None):
        ...
        if format_ in File.Extension.CSV:
            self.data.to_csv(filename)

        if format_ in File.Extension.PICKLE:
            pickle.dump(self, filename)

    @staticmethod
    def load(self, filename):
        '''
        Loads a bulbea.Share object from a csv/pickle file.
        '''
        _check_str(filename, raise_err = True)

        name, format_ = os.path.splittext(filename)
